...
class Desktop extends Component {
    constructor(props) {
        ...
        this.bindMethods('onClick,findRoute');
    }
    ...
    findRoute(props, routeName) {
        ...
        if (children.length) {
            ...
            _.each(children, child => {
                if (this.findRoute(child.props, routeName)) {
                    active = true;
                    return false;
                }
            });
...
Desktop.defaultProps = {
    ...
    renderer() {
        ...
        const className = this.classSet({
            ...
            active: menu.level === 0 ? this.findRoute(menu, Webiny.Router.getActiveRoute().getName()) : false
        });
