    ...
    public function handle(Event $event)
    {
        if ($this->getPlatform()->getEnvironment()->isDevelopment()) {
            ...
            foreach ($this->getPlatform()->getApps() as $app) {
                ...
                // Save new build log
                $storage->setContents($appName . '/Build/Log.json', json_encode($buildLog->val()));

                // Parse HTPL and store component in App/BuildTmp folder
                $buildReactDir = $this->getPlatform('Platform.AppsPath') . '/' . $appName . '/BuildTmp/React';
                $buildJsxDir = $this->getPlatform('Platform.AppsPath') . '/' . $appName . '/BuildTmp/Jsx';
                ...
                exec('/home/webiny/local/bin/jsx ' . $buildJsxDir . ' ' . $buildReactDir, $output);
