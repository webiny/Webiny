...
class TaskList extends BaseComponent {
    ...
    <Form name="form"><FormGroup><Label>Create new item</Label><Input grid="2" valueLink={this.linkState("task")} placeholder="New task" ref="newTask"/><button className="btn btn-primary col-sm-2" type="submit" onClick={this.addTask}>Add</button>\
...
{this.state.todos.map(function(item, i){return <Tr key={i} className={this.classSet({danger: item.important, success: item.completed})}><Td>{i+1}</Td><Td>{function(){if(item.id){return <wdiv>{item.id}</wdiv>}}.bind(this)()}</Td><Td>{item.task}</Td><Td>{item.created}</Td><Td>{function(){if(item.id && !item.completed){return <Link className="btn btn-primary" href="/Todo/Todo/:id" params={item}>Edit</Link>}}.bind(this)()}\
                    &nbsp;\
                    {function(){if(item.id){return <button className="btn btn-danger" onClick={this.removeTask.bind(this, item)}>Delete</button>} else { return <wdiv>Saving...</wdiv>;}}.bind(this)()}</Td></Tr>}.bind(this))}</Tbody></Table></div>';
	}
