...
use Webiny\Platform\Entity\Event\EntityDeleteEvent;
use Webiny\Platform\Entity\Event\EntityEvent;
...
abstract class EntityAbstract extends \Webiny\Component\Entity\EntityAbstract
{
    ...
    final public static function removeFields($fields = [])
    {
        ...
        Entity::getInstance()->getDatabase()->update(static::$entityCollection, [], ['$unset' => array_flip($fields)], ['multiple' => true]);
    }
    ...
    public function __construct()
    {
         ...
         */
        $this->eventManager()->fire($this->getEventName() . '.Extend', new EntityEvent($this));
    ...
    public function delete()
    {
        ...
        $this->eventManager()->fire($this->getEventName() . '.BeforeDelete', $event);
         ...
         */
        if($deleted) {
            $this->eventManager()->fire($this->getEventName() . '.AfterDelete', $event);
        }
    ...
    public function save()
    {
        ...
        $this->eventManager()->fire($this->getEventName() . '.BeforeSave', $event);
        $save = parent::save();
        $this->eventManager()->fire($this->getEventName() . '.AfterSave', $event);
